...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:9095/api/forgot', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setMessage(`Password reset token generated: ${data.message}`);
      } else {
        setError(data.message || 'Failed to send reset email');
      }
    } catch (err) {
      console.error('Forgot password error:', err);
      setError('Network error. Please check if the backend is running on port 9095.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-center">
            <Link
              to="/signin"
              className="inline-flex items-center space-x-2 text-sm font-medium text-orange-600 hover:text-orange-500"
            >
